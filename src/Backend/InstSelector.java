...
public class InstSelector implements IRVisitor {
    ...
    public void visit(IR.Function it) {
        ...
        for (int i = 0; i < Integer.min(6, it.param.size()); i++) {
        ...
        for (var reg : globalReg.keySet()) {
            currentBlock.inst.add(0, new la(globalReg.get(reg), reg));
        }
    ...
    public void visit(IR.Instruction.call it) {
        for (int i = 0; i < Integer.min(6, it.args.size()); ++i) {
